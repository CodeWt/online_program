...
$(function () {
    ...
    $('#runbtn').click(function () {
        var code = $('#runText').val()
        var obj = ajaxRequest("/run",code,false,"text/plain");
        if (obj.code==200){
            $('#consoleText').val(obj.data);
        }else {
            alert("run exeception!")
        }
    })
});
...
function createProject() {
    ...
    var userId = window.sessionStorage.getItem("userId");
    var obj = ajaxRequest("/treenode/create/proj", "{\"userId\":\""+userId+"\"}", false);
